...
class PersonVehicle(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "intpessoaid", "intveiculoid", name="uq_tblpessoaveiculo_pessoa_veiculo"
        ),
        {"mysql_engine": "InnoDB", "mysql_charset": "utf8mb4"},
    )

    id: Mapped[int] = mapped_column(
        "intpessoaveiculoid", primary_key=True, autoincrement=True
    )
    person_id: Mapped[int] = mapped_column(
        "intpessoaid",
        ForeignKey("tblpessoas.intpessoaid", name="fk_tblpessoaveiculo_pessoa", ondelete="RESTRICT"),
        nullable=False,
    )
    vehicle_id: Mapped[int] = mapped_column(
        "intveiculoid",
        ForeignKey("tblveiculos.intveiculoid", name="fk_tblpessoaveiculo_veiculo", ondelete="RESTRICT"),
        nullable=False,
    )
...
Index("idx_tblpessoaveiculo_intpessoaid", PersonVehicle.person_id)
Index("idx_tblpessoaveiculo_intveiculoid", PersonVehicle.vehicle_id)
